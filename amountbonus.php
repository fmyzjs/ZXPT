...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$point=$_POST['amount'];
	$class=$_POST['class'];
	if(!is_numeric($point))stderr("别玩了！", "填写的萝卜不是正常的数值！");
	...
	$query1 = sql_query("SELECT userid FROM forummods GROUP by userid");
		...
		{	sql_query("UPDATE users SET seedbonus = seedbonus + $point WHERE id=$dat[userid]");
			if(mysql_num_rows(sql_query("SELECT * FROM users WHERE id=$dat[userid]"))>0)sql_query("INSERT INTO messages (sender, receiver, added,  subject, msg) VALUES (0, $dat[userid], $dt, '福利来啦！', '管理组为所有"."$classname"."发放了$point"."个萝卜，请笑纳！')") or sqlerr(__FILE__,__LINE__);
		}
	}
	...
	$query3 = sql_query("SELECT * FROM users WHERE picker='yes'");
		...
		{	sql_query("UPDATE users SET seedbonus = seedbonus + $point WHERE id=".$dat[id]);
			if(mysql_num_rows(sql_query("SELECT * FROM users WHERE id=".$dat[id]))>0)sql_query("INSERT INTO messages (sender, receiver, added,  subject, msg) VALUES (0, $dat[id], $dt, '福利来啦！', '管理组为所有"."$classname"."发放了$point"."个萝卜，请笑纳！')") or sqlerr(__FILE__,__LINE__);}}
...
{	$classname="用户";
	...
	$classname=($class==10?"贵宾":($class==11?"养老族":($class==12?"保种员":($class==13?"发布员":($class==14?"管理员":($class==15?"站长":"主管"))))));
		...
		{	sql_query("INSERT INTO messages (sender, receiver, added,  subject, msg) VALUES (0, $dat[id], $dt, '福利来啦！', '管理组为所有"."$classname"."发放了$point"."个萝卜，请笑纳！')") or sqlerr(__FILE__,__LINE__);
		}
	}
//		write_log("25.0 bonus point is sent to everyone. -" . $CURUSER["username"] ,'normal');
		stderr("成功", "成功给所有$classname"."发放$point"."个萝卜");
		die;
	}
...
*/	$arr= mysql_fetch_assoc(sql_query("SELECT id, bonuscomment FROM users WHERE username=$username"));
	...
	print_r( $arr );
	if ( $seedbonus > 0 )
		$bonuschangedmsg = '站点为您增加了[color=red][b]'.$seedbonus.'[/b][/color]个萝卜，感谢您对本站的贡献！';
	elseif ( $seedbonus < 0 )
		$bonuschangedmsg = '您的萝卜被减少了[color=red][b]'.abs($seedbonus).'[/b][/color]个，请注意您在本站的言行！';
	else
		$bonuschangedmsg = '萝卜好像没有变化，今天不会是4月1日吧？！';
	if ( $reason )
	...
	sql_query("INSERT INTO messages (sender, receiver, added,  subject, msg) VALUES (0, $arr[id], $dt, '萝卜变动', ".sqlesc($bonuschangedmsg).")") or sqlerr(__FILE__,__LINE__);
...
begin_main_frame("调整某个特定用户的萝卜",false,50);
...
<tr><td class="rowhead">萝卜调整量：</td><td class="rowfollow"><input type="text" name="seedbonus" size="5"/></td></tr>

...
(
	...
	'tr_edit' => "编辑",
	'tr_res' => "我要应求",
	'id_not_exist' => "所求id不存在",
	'head_resreq' => "<h2>我要应求</h2>",
...
//	if
	...
	print("<tr>".
	"<td  width=3% class=\"rowhead_center nowrap\" valign=\"top\" >".$cat["name"]."</td>".
	"<td  width=40% class=\"rowhead_left nowrap\" valign=\"top\">{$man}<a href=viewrequest.php?action=view&id=".$row["id"]."><b>".$row["name"]."</b></a></td>".
	"<td  width=5% class=\"rowhead_center\" valign=\"top\" >".$row['ori_amount']."</td>".
	"<td  width=5% class=\"rowhead_center\" valign=\"top\"><font color=#ff0000><b>".($row['amount']-$row['ori_amount'])."</b></font></td>".
	"<td  width=7% class=\"rowhead_center\" valign=\"top\">".get_username($row['userid'])."</td>".
	"<td  width=4% class=\"rowhead_center\" valign=\"top\">".$row['comments']."</td>".
	"<td  width=4% class=\"rowhead_center\" valign=\"top\">".$count["count(*)"]."</td>".
	"<td  width=9% class=\"rowhead_center\" valign=\"top\">".gettime($row['added'],true,false)."</td>".
	"<td  width=8% class=\"rowhead_center\" valign=\"top\">".gettime($row['resetdate'],true,false)."</td>".
	"<td  width=8% class=\"rowhead_center\" valign=\"top\">".($row['finish']=="yes"?$lang_req['finished']:($row['finish']=="cancel"?"已撤消":($row['userid']== $CURUSER[id]?$lang_req['unfinished']:"<a href=viewrequest.php?action=res&id=".$row["id"]." >".$lang_req['unfinished']."</a>")))."</td>".
	(get_user_class() >= 13?"<td class=\"rowfollow\" valign=\"top\"><font color=black><a href=viewrequest.php?action=fastdel&id=".$row["id"]." >删</a> <a href=viewrequest.php?action=edit&id=".$row["id"]." >改</a> <a href=viewrequest.php?action=cancel&id=".$row["id"]." >撤</a></td>\n":"").
	...
	if($count_dispname > $max_length_of_torrent_name)
		$arr["name"]=mb_substr($arr["name"], 0, $max_length_of_torrent_name-2,"UTF-8") . "..";
...
"<input type=\"hidden\" name=\"pid\" value=\"".$id."\" /><br />");
	...
	$amount=$_POST["amount"];
	if($amount<1000)stderr("出错了！","发布求种赏金不得小于1000根胡萝卜！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	if($amount>10000)stderr("出错了！","发布求种赏金不得大于根胡萝卜！<a href=javascript:history.go(-1)>点击这里返回</a>",0);
	$amount += 100;
	...
	<input type=hidden name=reqid value=\"".$_GET["id"]."\" />
	请输入种子的id:（请根据地址栏填写）<br/>http://pt.bistu.edu.cn/details?id=<input type=text name=torrentid size=11/><br/>

...
// 		unset($imagehash);
// 		$imagehash = image_code () ;
// 		print ("<p>".$lang_functions['row_security_image']."</td>");
// 		print ("<p><img src=\"".htmlspecialchars("image.php?action=regimage&imagehash=".$imagehash)."\" border=\"0\" alt=\"CAPTCHA\" /></p>");
// 		print ("<p>".$lang_functions['row_security_code']."</p><p>");
// 		print("<input type=\"text\" autocomplete=\"off\" style=\"width: 180px; border: 1px solid gray\" name=\"imagestring\" value=\"\" />");
// 		print("<input type=\"hidden\" name=\"imagehash\" value=\"$imagehash\" /></p>");
// 	}
// }
...
function show_image_code () {
	...
	if ($iv == "yes") {
		unset($imagehash);
		$imagehash = image_code () ;
		print ("<p><div id=\"left\"><img  src=\"".htmlspecialchars("image.php?action=regimage&imagehash=".$imagehash)."\"  alt=\"CAPTCHA\" /></div>");
		print("<div id=\"right\"><input class=\"text2\" type=\"text\" autocomplete=\"off\"  name=\"imagestring\" value=\"\" placeholder=\"验证码\"/>");
		print("<input type=\"hidden\" name=\"imagehash\" value=\"$imagehash\" /></div></p></br>");
	}
}
...
<link rel="stylesheet" href="styles/common-components.css<?php echo $cssupdatedate?>" type="text/css" />
...
<table class="mainouter" width="982" cellspacing="0" cellpadding="0" align="center">
	<tr><td id="nav_block" class="text" style="border:0;" align="center">
<?php if (!$CURUSER) { ?>
		<div id="loginpanel">
			<a href="login.php">
				<font class="big">
					<b>
					<?php echo $lang_functions['text_login'] ?>
					</b>
				</font>
			</a>  ｜ 
			<a href="signupcard.php">
				<font class="big">
					<b>
						<?php echo $lang_functions['text_signupcard'] ?>
					</b>
				</font>
			</a> ｜ 
			<a href="signup.php?type=invite">
				<font class="big">
					<b>
						<?php echo $lang_functions['text_invite_reg'] ?>
					</b>
				</font>
			</a> ｜ 
			<a href="signup.php">
				<font class="big">
					<b>
						<?php echo $lang_functions['text_signup'] ?>
					</b>
				</font></a>
				...
				<tr>
					<td class="bottom" align="left">
						<span class="medium">
							...
							{ ?> [<a href="settings.php"><?php echo $lang_functions['text_site_settings'] ?></a>]<?php }
							?> 
							...
							<?php echo number_format((int)$CURUSER['seedbonus'], 0)?> 
							<font class = 'color_invite'><?php echo $lang_functions['text_invite'] ?></font>
							[<a href="invite.php?id=<?php echo $CURUSER['id']?>"><?php echo $lang_functions['text_send'] ?></a>]: 
							<?php echo $CURUSER['invites']?><br />

							<font class="color_ratio"><?php echo $lang_functions['text_ratio'] ?></font> 
							<?php echo $ratio?>  
							<font class='color_uploaded'><?php echo $lang_functions['text_uploaded'] ?></font> 
							<?php echo mksize($CURUSER['uploaded'])?>
							<font class='color_downloaded'> <?php echo $lang_functions['text_downloaded'] ?></font> 
							<?php echo mksize($CURUSER['downloaded'])?>  
							<font class='color_active'><?php echo $lang_functions['text_active_torrents'] ?></font> 
							<img class="arrowup" alt="Torrents seeding" title="<?php echo $lang_functions['title_torrents_seeding'] ?>" src="pic/trans.gif" />
							<?php echo $activeseed?>  
							<img class="arrowdown" alt="Torrents leeching" title="<?php echo $lang_functions['title_torrents_leeching'] ?>" src="pic/trans.gif" />
							<?php echo $activeleech?>&nbsp;&nbsp;
							<font class='color_connectable'><?php echo $lang_functions['text_connectable'] ?></font>
							<?php echo $connectable?> <?php echo maxslots();?>
						</span>
					</td>
					<td class="bottom" align="right">
						<span class="medium">
							<?php echo $lang_functions['text_the_time_is_now'] ?>
							<?php echo $datum[hours].":".$datum[minutes]?>
							<br />
...
while ($row = mysql_fetch_assoc($res))
{
	...
	print("<td class=\"rowhead_right\" width=\"100%\" align=\"left\"><table class=\"torrentname\" width=\"100%\"><tr" . $sphighlight . "><td class=\"embedded\">".$stickyicon."<a $short_torrent_name_alt $mouseovertorrent href=\"details.php?id=".$id."&amp;hit=1\"><b>".htmlspecialchars($dispname)."</b></a>");
	//判断种子免费置顶时间到期
	...
	print("<td class=\"rowhead_center nowrap\">". $time. "</td>");

	//size
	print("<td class=\"rowhead_center\">" . mksize_compact($row["size"])."</td>");
	...
	if ($row["seeders"]) {
		...
		print("<td class=\"rowhead_center\"><span class=\"" . linkcolor($row["seeders"]) . "\">" . number_format($row["seeders"]) . "</span></td>\n");

	if ($row["leechers"]) {
		print("<td class=\"rowhead_center\"><b><a href=\"details.php?id=".$id."&amp;hit=1&amp;dllist=1#leechers\">" .
		number_format($row["leechers"]) . "</a></b></td>\n");
	}
	else
		print("<td class=\"rowhead_center\">0</td>\n");

	if ($row["times_completed"] >=1)
	print("<td class=\"rowhead_center\"><a href=\"viewsnatches.php?id=".$row[id]."\"><b>" . number_format($row["times_completed"]) . "</b></a></td>\n");
	else
	print("<td class=\"rowhead_center\">" . number_format($row["times_completed"]) . "</td>\n");

		if ($row["anonymous"] == "yes" && get_user_class() >= $torrentmanage_class)
		{
			print("<td class=\"rowhead_center\" align=\"center\"><i>".$lang_functions['text_anonymous']."</i><br />".(isset($row["owner"]) ? "(" . get_username($row["owner"]) .")" : "<i>".$lang_functions['text_orphaned']."</i>") . "</td>\n");
		}
		elseif ($row["anonymous"] == "yes")
		{
			print("<td class=\"rowhead_center\"><i>".$lang_functions['text_anonymous']."</i></td>\n");
		}
		else
		{
			print("<td class=\"rowhead_center\">" . (isset($row["owner"]) ? get_username($row["owner"]) : "<i>".$lang_functions['text_orphaned']."</i>") . "</td>\n");
		}

	if (get_user_class() >= $torrentmanage_class)
	{
		print("<td class=\"rowhead_center\"><a href=\"".htmlspecialchars("fastdelete.php?id=".$row[id])."\"><img class=\"staff_delete\" src=\"pic/trans.gif\" alt=\"D\" title=\"".$lang_functions['text_delete']."\" /></a>");
